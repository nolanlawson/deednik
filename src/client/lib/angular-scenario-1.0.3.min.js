...
angular.scenario.dsl('input', function() {
  ...
  chain.setChecked = function(value) {
      return this.addFutureAction("checkbox '" + this.name + "' setChecked '"+ value + "'", function($window, $document, done) {
          var input = $document.elements('[ng\\:model="$1"]', this.name).filter(':checkbox');
          input = value ? input.not(':checked') : input.filter(':checked');
          input.trigger('click');
          done();
      });
  };
